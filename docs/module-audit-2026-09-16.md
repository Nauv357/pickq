...
| C15 | 题库子页面折叠侧栏的正则漏了 `/banks/:id/notes`（同是题库子页，笔记页却留着宽侧栏） | 规则统一：题库子页（详情/做题/历史/笔记）都折叠侧栏（**后续**：笔记页已升为顶层页 `/notes`，该正则只剩详情/做题/历史） |
| C16 | 笔记**硬绑**在题库/题目上（`note.bank_id` + `note.question_id`），一条体会想同时挂到两道同类题上只能复制一遍 | 笔记独立成实体 + `note_link` 关联表（0..N 个题库、0..N 道题，未归类也合法）；侧边栏加「笔记」顶层入口；删库/删题只删关联、保留笔记 |
| C17 | 练习**成绩报告**与**练习历史**是两套实现，同一场练习两个入口看到的东西不一样（成绩页展开只给答案、题干要往上滚） | 抽出 `SessionReview.vue`，两处渲染同一个组件、同一份数据（`GET /sessions/{id}`）；主观题自评也只剩一处实现 |
| C18 | 笔记的渲染框与编辑框都是固定高度、不能拉大，短笔记也要滚动 | 输入框跟着内容长高（上限 400px 后内部滚动）；渲染与编辑同一套富文本规则；含公式/图片/表格时才给一块实时预览 |
...
## 六、后续修复（C16–C18 那一批，2026-09-16 当天完成）

- 数据库：新增 `V22__note_links.sql`（建 `note_link` → 平移旧归属 → 删 `note.bank_id`/`question_id`）。
  升级自动迁移，旧笔记一条不丢；删库/删题改为只删关联行。
- 组件：`SessionReview.vue` 同时服务成绩页与练习历史；`SessionHistoryView` 1000 → 282 行，
  `PracticeView` 的报告段删掉重复的主观题自评与折叠明细（约 -300 行）。
- 接口：`NoteController` 由 5 个端点（挂在题库路径下）改为 7 个端点（`/api/notes` 为入口，
  含 `POST/DELETE /api/notes/{id}/links`）。
- 验证（本轮实测）：后端 **260 通过 / 0 失败**（新增 `ContentPackageAppendTest` 5 项、
  `NoteServiceTest` 重写为 9 项）；`smoke:bank` **80**、其余冒烟同前；`check:ui` 与 `build` 通过；
  真后端 `e2e:tutor` / `e2e:skills` 见提交说明。
