...
- **`module-audit-2026-09-16.md`** — **功能与模块分配审计**：同一套业务口径被复制成多份（错题判定 5 份且互不相等）、死代码、入口错位与文档漂移的清单，分**必修（已改）/ 建议 / 记录**三级。**加功能前先扫一眼它的"建议"段**，避免重复造同一件事。
- **`plaza-redesign-requirements.md`** — 广场改版需求（独立线，未启动）。
