...
**保留的内聚算法簇：** PDF 图形块识别、页面坐标、图像裁剪与选项格切分仍在主服务内。这些方法共享同一份 PDF 页面渲染、坐标系、文本位置和图片缓存；若仅为降低行数而拆成多个 Service，会引入大量可变上下文参数，并降低调试效率。后续仅在引入独立 `VisionLayoutContext`（不可变页面上下文）后，再整体迁为 `AiImportVisionLayoutService`；不得继续在主服务中添加新的业务规则。
