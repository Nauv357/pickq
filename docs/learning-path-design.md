...
> 状态：**阶段 0–3 已实现；阶段 3.5（2026-09-16）按产品复核做了大幅收口**——
> 路线页/目标/每日任务/抽测/闪卡已从界面撤下，规划改为隐形进「开始练习」的配题引擎，
> 掌握度只用于排序挑题、不再宣判。**先读 §11 阶段 3.5 的实施记录，再看其余章节**
> （§5.3 门控、§6 五层护栏、§7 路线与练习形态里凡涉及"对用户展示路线/掌握度"的部分，都以收口记录为准）。
> 已确认的三个前提（用户拍板）：
...
把"题库"从**一堆题**变成**一条能走的练习流**：目标 → 路线 → 你的缺口 → 今日任务 → 做题/追问 → 掌握度 → 路线自适应。
错题追问是这条链上每一环的入口，不是独立功能。

**要解决的问题**：用户导入题库 → 不知道还缺什么 → 刷两题 → 放弃。

> **收口后的口径（2026-09-16）**：这条链只有 ③④⑤ 留在界面里，而且被压成**一次点击**
> （「开始练习」= ③缺口 + ④定序 + 一句事实解释）；①目标、②路线、⑥掌握度回馈
> **不再作为界面存在**，只在后台参与挑题。理由见 §11 阶段 3.5。
...
- **必须把该节点下的单题全部列出来**（题号 / 题型 / 题干 / 置信度），并支持逐题「确认此题 / 改挂 / 丢弃」。
  只给三条样例题干是不够的——用户实测反馈"每个分类下只简略显示题目简单信息，我根本分辨不出来是哪一题，
  在这个界面可以说完全无法对题目精确分配 tag"。逐题操作与整节点操作（全部确认 / 改挂 / 全部丢弃）并存：
  几十题仍然 <1 分钟（批量），但需要精修时不必来回跳页面。
- **覆盖口径只允许有一套**：`已确认 + 待确认 + 未匹配 = 总题数`，三段互不重叠；界面上的每个数字
  都必须等于按该状态查出来的清单条数（`GET /banks/{id}/skills/questions?status=`）。
  2026-09-15 真实数据验收时出现过「26 待标注 vs 16 待确认」并列展示——两个口径（"没有可用标签的题数"
  与"未确认建议按节点聚合后的条目数"）混在一起，用户无法判断还剩多少题没处理。另设一个单独口径
  「可用于判定掌握」= 已确认 **或** AI 置信度 ≥ 0.8。
- **未匹配的题要能逐题补**：单独一个「未匹配」页签全列出来，逐题「打开题目」进编辑器指定；
  也可用「设为主题」把 AI 的分组名写回题目 `topic`（只填空值）。
- 编辑器里的「知识点」多选是**并列的修改入口**（来源标注为 你标注的 / 作者标注 / AI 建议）；
  从弹窗点「打开题目」进来时该栏要滚到视野中间并高亮——用户实测反馈"确认后找不到可以修改每题 tag 的地方，
  甚至编辑页面都没有显示 tag"（后者是真 bug：面板在 setup 阶段就回填了详情，加载技能图/标签的 watch
  被 `initialFilled` 短路，一次都没执行）。
...
| 阶段 | 内容 | 验收标准 | 依赖 |
| --- | --- | --- | --- |
| **0 标签地基** | 技能图 + question_skill + **逐题判定（默认）+ 分组映射（有 topic 时）** + 待确认队列（逐题可改）+ 覆盖地图 | 抽样准确率 ≥90%（当前真实数据只有 212 题，先按用户题库的 172 题做）；同类题标签一致率 100%；**覆盖三段相加 = 总题数，且概览数字 = 按状态查出的清单条数** | 无 |
| **1 追问入口** | 答错即问 + 提示楼梯 + 事后抽屉 + 复盘诊断 + **流式输出** | 复盘诊断能指出薄弱节点；追问上下文完整；流式首字 <2s | 阶段 0（诊断需要节点标签） || **2 只读路线** | 模板（3–5 个官方）+ 外缘 + 缺口清单 + 每日任务（不自动出题） | 用户能看懂"今天做什么""还缺什么"；日常零 token | 阶段 0 |
...
0b. **已完成（2026-09-15）**：125 题公考真题实测 → 覆盖 79%（技能图偏粗，已按 §4.2 拆细到 32 个节点）；
   同一次实测暴露的两个真问题已修：覆盖面口径混用（见 §4.3）、编辑页知识点拿不到技能图（同上）。
   验证手段固化为 `frontend/scripts/smoke-skills.mjs`（假后端，35 项）与
   `frontend/scripts/e2e-skills-live.mjs`（真后端 + 假模型，32 项，逐项与后端口径对账）；
0c. **阶段 1 已实施（2026-09-15）**：提示楼梯（L1/L2/L3）+ 答错即问（错因三选）+ 整场复盘诊断 + **SSE 流式输出**；
   落库 `tutor_session`（含 `self_reason`）/ `tutor_message`（含 `hint_level`），V18 迁移。
   实现要点与界面入口见 `docs/features.md` §4.6；验证手段为 `TutorServiceTest`（6 项，含流式降级与固定上下文顺序）
   与 `frontend/scripts/e2e-tutor-live.mjs`（真后端 + 假流式端点，17 项）。
   **与设计的一处偏差**：本应用是"交卷统一判分"，做题时并不知道对错，所以"答错即问"放在交卷后的错题里，
   做题中提供的是"提示楼梯"（不剧透答案）——功能等价，入口位置按真实交互调整；
   §7.4 入口③（错题本"问老师"）与 §7.5 闪卡属阶段 3，尚未做。
0d. **阶段 2 已实施（2026-09-15）**：外缘 + 缺口清单 + 每日任务 + 路线图页（V19 迁移：`learner_profile`、`daily_task`、
   `practice_session.scope_node`）。**完全不调模型**（零 token），公式落在 `RoadmapService`：
   掌握度/门控按 §5.2、§5.3 实现，`user_skill_state` 刻意不落表（避免两份真相）。
   会话新增 `mode=SKILL`（按知识点抽题），支撑"一键开始今天的任务"。
   实现要点见 `docs/features.md` §4.7；验证为 `RoadmapServiceTest`（5 项：证据不足/在练/过关/提示打折/前置未满足/题单冻结）
   与 `frontend/scripts/e2e-roadmap-live.mjs`（真后端、**故意不配 AI**，19 项）。
   **对 §7.1 的一处补强**：外缘在"阶段顺序"之外增加"**有题可练优先**"——否则在只标了一部分知识点的题库上，
   路线会指着一个空节点、"今天做什么"直接为空。这条在测试里锁住了。
1. 人工给 200 道真实真题打知识点标签 → 与 AI 结果比对（决定后面一切是否可靠）；
2. 3–5 人试用"做完一场 → 收到诊断 → 追问"（验证留存假设）；
3. 拿 10 个高频目标对照真实题库，估算模板命中率与缺口规模。

### 阶段 3 实施记录（2026-09-15）

0e. **阶段 3 已实施**：抽测降级（`REGRESSED`）+ 闪卡（生成/确认/复习调度）+ 自适应难度（85% 规则）。
   V20 迁移：`card`、`question_difficulty`。实现要点见 `docs/features.md` §4.8。
   验证：`AdaptiveServiceTest`（7 项）、`CardServiceTest`（6 项）、`RoadmapServiceTest` 新增
   "抽测降级 / 抽测安排 / 题库隔离"（8 项）、`frontend/scripts/e2e-cards-live.mjs`（真后端 + 假模型，16 项）。
   **三处与设计的偏差（都是刻意的）**：
   - 抽测降级**不落表**：由"过关之后的失败证据"推导（设计里隐含需要一个状态表）；
   - 闪卡调度**沿用题目那套阶梯**，不引入 FSRS 的 stability/difficulty（用户不该学两种复习节奏）；
   - `θ_u` 能力分**不落表**（由知识点正确率映射），只把每题难度落库——它才是跨请求需要保留的量。
   **实测抓出的 bug**：`daily_task` 最初漏了 `bank_id`，两个题库会共用同一份"今天的题单"
   （换题库看路线时题单串库、完成度永远 0）。当前冻结必须按题库冻结，已补迁移与用例。

### 阶段 3.5 收口记录（2026-09-16）：路线 → 配题，掌握度 → 不宣判

0f. **产品复核后的自我否定**：阶段 1–3 做完，用户（也是第一个真实使用者）提出两个问题，
   都成立，因此**砍掉了一大半界面**（详细清单见 `docs/features.md` §4.8）：
   - **"感觉有一种和做题学习的隔离感"**：路线页要求用户先做一轮元活动（看计划、设目标、读"下一步"），
     才能开始练。用户打开题库想干的事只有一件——**开始练**；把"规划"与"做题"拆到两处，
     规划就变成了负担。**规划必须隐形。**
   - **"我自己使用时都不确定这么做是否正确"**：可信度问题。一条学习路线要可信，需要
     内容（教材/讲义）+ 大纲（考什么）+ 诊断 + 定序 + 教学 + 验证；我们只有**诊断 + 定序**，
     外加一点点教学（AI 讲解）。**本质是题库，无法提供学习资料**，所以"你已掌握 / 你该学什么"
     这类结论我们没资格下——一旦判错，整套系统的可信度归零。
0g. **收口原则（写下来供以后判断功能去留）**：
   - **一个功能如果不能在 3 分钟内让用户感到价值，它就在消耗用户的耐心**；
   - **只承诺用户能自己核对的事**（错过几次、什么时候到期、这个知识点你有几道题），
     绝不出现"掌握度/你应该/建议你/你需要加强"这类宣判（有测试断言 explain 不含这些词）。
0h. **落地方式**：把"路线"降级成一次配题（`PracticePlanService`）——
   `GET /practice-plan` 按「错题 30% → 到期 30% → 薄弱知识点 30% → 新题补满」去重取题，
   会话用 `mode=PLAN` + `questionIds` 原样练，练习页顶部给**一句可核对的事实解释**。
   界面撤掉：学习路线页、目标设定、每日任务、抽测、闪卡；题型头部的入口一并下线。
   引擎保留在后台（掌握度公式、85% 规则、间隔重复、AI 私教），**掌握度只用于排序挑题，不再展示也不喂模型**。
0i. **验证**：`PracticePlanServiceTest`（10 项：桶顺序/去重/解释口径/薄弱点选择/确定性/题量收敛/题库隔离）、
   `PracticeSessionPlanModeTest`（4 项：原样顺序、忽略 count、丢弃未知 id、空 ids 报错可读）、
   `TutorServiceTest` 新增讲解 2 项；前端 `smoke:bank` 新增「头部只有 2 个按钮 / 更多菜单 / 一键配题 → PLAN 会话 → 说明横幅可关」。
   **实施中抓出的两个问题**：
   - `floor(0.3 × want)` 在短练习（want ≤ 3）时把三个桶全抹成 0，于是"练 1 题"永远只给新题，
     与"错题优先"的口径自相矛盾 → 按优先级给保底 1 题；
   - 节点显示名一度直接用 `nodeId`（用户会看到 `gk.zl.growth`）→ 名称统一取自技能图，取不到才退回 id。
0j. **下一步投入方向**（按价值排序）：① 错题讲解做到最好（本轮完成）；
   ② 缺口补题（按缺口用 AI 出题 → 预览确认入库）；③ AI 导入继续打磨；④ 广场质量信号。
   **长期答案**：路线的可信度不靠自己造，靠"谁给的"——将来由名师/机构在广场提供路线模板，
   我们只做执行器，并且给用户**覆盖清单（事实）**而不是时间表。
