...
② 一次 AI 调用：把"唯一值列表 + 技能图节点表"映射成 {topic → nodeId[]}
   —— 同时给每个节点产出 3–8 个 keywords（用于以后新题自动归组）
③ 同 topic 的题继承映射结果（保证同类题标签一致）
④ 无 topic/category 的题：AI 先提议 20–30 个分组（一次调用）→ 再分批归组
⑤ 单题兜底：题干 + 解析 + 现有 topic，AI 判定 1–4 个节点（低置信 → 待确认）
```

**为什么必须两级**：逐题判定会出现"同类题标签不一致"（最危险的漏刷来源）；按 topic 成组映射把同类题钉在同一个节点上，错也只错一组，且用户一眼能看出来。
...
| 阶段 | 内容 | 验收标准 | 依赖 |
| --- | --- | --- | --- |
| **0 标签地基** | 技能图 + question_skill + 两级映射 + 待确认队列 + 覆盖地图 + 标签抽样工具 | 抽样 200 题准确率 ≥90%；同类题标签一致率 100% | 无 |
