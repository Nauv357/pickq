...
## 4.6 AI 私教：提示楼梯 / 答错即问 / 复盘（学习路径引擎阶段 1）

设计见 `docs/learning-path-design.md` §7.3、§7.4、§9。**五处 AI 调用里的"追问对话"与"复盘诊断"就是它。**

三条入口，都在用户主动开口时才说话（**做题过程绝不自动弹窗**）：

| 入口 | 位置 | 做了什么 |
| --- | --- | --- |
| 提示楼梯 | 做题中 · 题目卡右上「提示」 | L1 只指方向 → L2 关键一步 → L3 完整解析；已给过的级别标出来，逐级解锁 |
| 答错即问 | 交卷后 · 错题展开里的「这题为什么错？」 | 先选错因（看错/蒙的 · 这个知识点不会 · 完全没见过）→ 针对错因给一句关键提醒；可继续追问 |
| 复盘诊断 | 交卷后 · 成绩页顶部「这场复盘」 | 先给**公式统计**（成绩 + 按知识点的错题分布 + 该知识点历史正确率），再点按钮让 AI 讲一遍 |

- **本应用是"交卷统一判分"**，做题时还不知道对错，所以"答错即问"放在交卷后的错题里；做题中给的是**提示**（不剧透答案）。
...
- **薄弱点排序是公式算的**（本场错题按知识点聚合 + 历史正确率），模型只负责把结论讲成人话；复盘 prompt 明确要求
  "只写清单里的题，不要编造"。
- **流式输出**：`POST /api/tutor/{hint|ask|review}` 是 SSE（事件 `session` / `delta` / `done` / `error`），
  边生成边显示；模型网关不支持流式时后端自动降级为一次性返回（体验退化但不失败）。
- **落库**：`tutor_session`（含 `self_reason` 错因 —— 比模型猜准得多的第一手数据）与 `tutor_message`
  （助手每条带 `hint_level`）。同一题 + 同一场练习的会话会复用，重新打开能接着上次聊。
  这些记录将来是掌握度的**负向证据**（要了提示才做对 ⇒ 同样的"对"更不值钱，见设计 §5.1、§7.2）。
- 未配置 AI 时面板给可照做的提示（去设置里填 Key），不影响做题与判分。

## 4.7 学习路线：今天做什么 / 下一步 / 还缺什么（学习路径引擎阶段 2）

题库详情页 →「学习路线」。设计见 `docs/learning-path-design.md` §5.2、§5.3、§7.1、§7.2。

**这一层完全不调模型**（日常零 token）：所有结论都是可复现的公式，刷新多少次都一样，没配 AI 也能用。

- **掌握度公式**（`RoadmapService`，与设计 §5.2 一致）：
  `independence = clamp(1 − 0.15×提示级数 − 0.25×是否追问)`
  `evidence = 对错 × independence × exp(−距今天数/30) × (间隔≥7 天且做对 ? 1.2 : 1)`
  `mastery = Σ(evidence×w)/Σ(w)`，取最近 8 次作答（近 3 次权重 1.5）。
- **过关门控**（§5.3）：节点题量 ≥3（否则 `UNVERIFIED`，显式提示"证据不足"而不是假装掌握）
  ＋ mastery ≥ 0.85 ＋ 独立正确 ≥3 次 ＋ 独立作答比 ≥0.70 ＋ 间隔 ≥7 天复测仍正确（`spacedOk`）。
  状态：`CLEARED` / `LEARNING` / `UNVERIFIED`。
  **提示与追问是负向证据**：要过提示才做对的题不计入"独立正确"，也不给满分独立度。
- **外缘（下一步）**：前置全部 `CLEARED` ＋ 自己未 `CLEARED` ＋ 非可选节点；
  按（阶段顺序, 权重降序, 题量降序）取前 3。**额外约束：优先"你确实有题可练"的节点**——
  否则路线会指着一个空节点、"今天做什么"直接是空的；一个都没有时仍给出方向并提示去补题。
- **缺口**：题量为 0 或 <3 的节点全部列出（防漏刷第 5 层护栏），点「去补题」回题库页。
- **今日任务**：外缘第一个节点的 N 道题（优先没做过的）＋ 到期复习题（复习计划开启时）。
  N = `learner_profile.daily_questions`（默认 20）。**当天冻结**（落 `daily_task`）：答掉一题清单不变样；
  完成度按**当天的实际作答**现算（不存额外状态，删掉作答记录就自动回退）。
- **一键开始练习**：会话新增 `mode=SKILL` + `nodeIds`（按知识点抽题），题与节点的关联走 `question_skill`，
  与知识点页、标签审阅共用同一份标签。
- **个体输入**（`learner_profile`）：目标模板 / 目标原话 / 每日题量（1–200 夹取）/ 时间预算（0 = 不设）。
  首次自评问卷与目标时间点先留位，阶段 3 用。
- **不落的表**：`user_skill_state` 故意不建——`study_record` + `question_skill` + `tutor_message` 已是全部证据，
  再存一份就会出现"两份真相"（删掉作答记录后状态对不上）。现算是 O(该题库作答数)，本地数据量下毫秒级。

## 4.8 掌握度闭环：抽测降级 / 闪卡 / 自适应难度（学习路径引擎阶段 3）

设计见 `docs/learning-path-design.md` §5.4、§5.5、§7.5。**这一层同样不调模型**（只有"生成闪卡"那一次会用到 AI）。

### 抽测降级（假掌握检测）

- **安排**：已 `CLEARED` 的节点，距最近一次作答超过间隔阶梯（`7 → 21 → 60` 天，按"过关后练过几次"往后走）
  → 今日任务里出现一条 `SPOT_CHECK`（抽 1–2 题）。一天最多安排一个，别把复习变成考试。
- **降级规则**（纯公式，判定在 `RoadmapService.computeNode`）：
  找出"最近一次满足过关条件"的作答时间 `clearedAt`（独立做对且距今 ≥7 天），
  若之后出现**失败证据** → `REGRESSED`：
  ① 这道题又做错了；② 该题的**闪卡自评「忘了」**。
  只要节点**曾经**满足过关的结构条件（独立正确 ≥3、有间隔复测、题量 ≥3），降级就成立——
  即使当前 mastery 已被这次失败拉低，也如实标成「抽测掉下来了」而不是含糊地退回"在练"。
  降级后掌握度按 ×0.6 展示，并重新进入外缘（要求重练）。
- **不落表**：抽测结果就是普通的作答记录，降级由证据推导（见 §4.7 的"不落的表"理由）。

### 闪卡（题目测"再认"，卡片测"回忆"）

- **生成**（`POST /api/cards/generate`，AI，20 题一批）：只处理"有解析（≥20 字）且还没有卡"的题，
  一次调用上限默认 5 次、可中断续跑；模型返回的 `questionId` 必须属于本批次（防编造），
  单题最多 3 张卡。提示词要求挖"记住就能做对"的东西（公式/结论/易错点/判别口诀），`front` 必须能独立看懂。
- **确认门槛**：AI 生成的卡 `confirmed=0`，**未确认不参与调度**（`due_at` 为空）；
  手动编辑过的卡视为人工确认并立刻进队列。
- **调度**：与题目**同一套阶梯**（答对 `level+1`、间隔 `min(2^level,30)` 天；答错 `level=0`、立刻重来、`lapses+1`）。
  设计里写的是 FSRS 参数（stability/difficulty），这里刻意统一成题目那套：用户不该学两种复习节奏。
- **可溯源**：每张卡带 `question_id` + 出处题号，列表里可点回原题。
- **只作抽测证据**：卡片答对**不足以**把节点判为已掌握；答错能让已过关的节点掉回 `REGRESSED`。

### 自适应难度（85% 规则）

- **每题难度**（`question_difficulty`）：初值按题型估计（主观题 520 / 多选 460 / 单选 400 / 判断 320，
  客观题分值 >2 再 +40）；之后每次作答按 Elo-lite 更新 `d_q ← d_q − K×(实际 − p)`（K=16）。
- **能力分** `θ`：由"该题所属知识点上的历史正确率"映射（做了贝叶斯收缩，3 次先验，样本少时向 0.5 靠），
  **不落表**——它就是掌握度的单调映射。
- **选点**：`orderByTargetSuccess` 把候选题按"预测成功率是否落在 80%–90%"排序，`今天做什么`取前 N 题。
- **交错混练**：`interleaveRatio(mastery)` = 0（<0.6）/ 0.15（≥0.6）/ 0.35（≥0.85），
  掌握之后混入前置/后继节点的题；低掌握度时纯集中练（设计：交错对低基础者有害）。
- 作答路径上的更新失败**不影响**作答记录落库（难度只是选点依据，不是判定结论）。

### 当日任务与题库隔离

`daily_task` 带 `bank_id`，唯一键是 `(bank_id, task_date, kind, node_id)`。
**这条是实测抓出来的 bug**：最初漏了 bank_id，用户有两个题库时，换题库看路线会看到上一个题库今天的题单，
完成度永远是 0——所以"当天冻结"必须按题库冻结。
