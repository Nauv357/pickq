...
（设计见 `docs/learning-path-design.md`）。

- **技能图**：受控词表（节点 + 前置关系），内置两张官方模板——「公务员行测（通用）」「AI 时代的现代化全栈」；
  节点变更走远端配置通道（不发版），加载时校验悬空前置/重复 id/前置环。
- **打标签**：AI 按批提建议（默认逐题判定；题库若填了 `topic`/`category` 则优先按组映射，同类题标签天然一致），
  结果落 `question_skill`（`source=ai, confirmed=0`）。**分批推进**：界面一次只花 N 次模型调用，
  可中断可续跑（已处理的题不会重复问模型）。
- **人工优先**：建议按节点聚合展示，**并把该节点下的单题全列出来**（题号 + 题型 + 题干 + 置信度），
  可逐题「确认此题 / 丢弃建议 / 打开题目」，也可整节点「全部确认 / 改挂 / 全部丢弃」；
  用户手动标注覆盖 AI 与作者，且被人工标注过的题不再接受 AI 建议。
- **覆盖口径（三段不重叠）**：`已确认 + 待确认 + 未匹配 = 总题数`。界面上的每个数字都必须等于
  按该状态查出来的清单条数（`GET /banks/{id}/skills/questions?status=`），避免"概览数字"和"清单"两套口径打架。
  另一个口径「可用于判定掌握」= 已确认 **或** AI 置信度 ≥ 0.8 的题数，单独展示。
- **未匹配清单**：AI 没给出可用知识点的题单独一个页签全列出来，逐题「打开题目」去补；
  也可用「设为主题」把 AI 的分组名写回题目主题（只填空值，不覆盖作者已写的内容）。
- **编辑页也能改**：题目编辑器里有「知识点」多选框（来源标注为 你标注的 / 作者标注 / AI 建议），
  从标签弹窗点「打开题目」进来时该栏会滚到视野中间并高亮——用户实测反馈过"找不到改 tag 的地方"。
- **覆盖地图**：按阶段列出每节点的题量，显式标出「你没有这个知识点的题」「题太少（<3 题）暂不能判定掌握」——
  防"以为刷完了其实漏了"。**没有标签的题照常进练习与复习调度**，缺标签只影响知识点维度的统计。
