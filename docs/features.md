...
题库详情页 →「知识点」：把题目对应到**技能图**上的知识点，是学习路线、缺口分析与掌握度判定的地基
...
- **技能图**：受控词表（节点 + 前置关系），内置两张官方模板——「公务员行测（通用）」「AI 时代的现代化全栈」；
  ...
  **用户可以加自定义知识点**：在标签下拉里直接输入新名称即创建（落 `{dataDir}/skill-custom-nodes.json`，
  挂在「自定义知识点」阶段下；同名复用、可删）。`graphVersion` 随自定义节点变化，缓存的分组映射自动失效重算。
...
- **门控与练习**：只有「已确认」或「AI 置信度 ≥ 0.8」的标签参与掌握度判定；低置信标签只作参考。
  **没有标签的题照常进练习与复习调度**，缺标签只影响知识点维度的统计。
