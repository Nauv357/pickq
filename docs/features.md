...
| AI 私教 | 错题讲解（三段）/ 提示楼梯 / 追问 / 整场复盘 | `PracticeView.vue` | `TutorController` / `TutorService` |
...
## 4.6 AI 私教：错题讲解 / 提示楼梯 / 复盘（学习路径引擎阶段 1）

设计见 `docs/learning-path-design.md` §7.3、§7.4、§9。**五处 AI 调用里的"错题讲解/追问"与"复盘诊断"就是它。**

入口都只在用户主动开口时才说话（**做题过程绝不自动弹窗**）。**主线是「错题讲解」**：

| 入口 | 位置 | 做了什么 |
| --- | --- | --- |
| **错题讲解（主线）** | 交卷后 · 错题展开里的「讲给我听」 | 一次讲清三段：**错在哪**（针对你选的选项）→ **这类题怎么做**（可复用的做法）→ **下次防错**（一句能执行的检查）；可先点错因（看错/蒙的 · 这个知识点不会 · 完全没见过，**可选**）；讲完可就地追问，或「存为解析」写进题库 |
| 提示楼梯 | 做题中 · 题目卡右上「提示」 | L1 只指方向 → L2 关键一步 → L3 完整解析；已给过的级别标出来，逐级解锁 |
| 复盘诊断 | 交卷后 · 成绩页顶部「这场复盘」 | 先给**公式统计**（成绩 + 按知识点的错题分布 + 该知识点历史正确率），再点按钮让 AI 讲一遍 |

- **为什么讲解是主线**：用户点开错题时想要的是"我为什么错、下次怎么不出错"，一次给完（三段可扫读）
  比"先选错因、再问一句、再打开抽屉追问"的多次往返更值；错因三选降级成**可选的细化**，不选也能讲。
- **讲解与提示楼梯是两件事**：楼梯是"做题中还不想看答案"的分级给（占 `hint_level`）；
  讲解是"已经错了、要弄明白"的一次性交付（**不占提示级别**），但会作为对话历史参与后续追问。
- **讲解讲的不是通用解析**：上下文里带着「我的作答 / 我的错因 / 这道题错过几次 / 这个知识点上的历史表现」，
  所以能说"你选 B 是因为把基期当成了现期"；官方解析只作参考，prompt 明确要求**讲透为什么，不要照抄**。
- **本应用是"交卷统一判分"**，做题时还不知道对错，所以错题讲解放在交卷后；做题中给的是**提示**（不剧透答案）。
...
- **流式输出**：`POST /api/tutor/{explain|hint|ask|review}` 是 SSE（事件 `session` / `delta` / `done` / `error`），
  ...
  讲解的前端按三个小标题**分块渲染**（`【错在哪】/【这类题怎么做】/【下次防错】` 原样输出），
  边写边逐块长出来，不用等整段生成完。
- **落库**：`tutor_session`（含 `self_reason` 错因 —— 比模型猜准得多的第一手数据）与 `tutor_message`
  （助手每条带 `hint_level`，讲解为空）。同一题 + 同一场练习的会话会复用，重新打开能接着上次聊。
- **「存为解析」**：讲解可直接写进这道题的 `analysis`（走题目解析保存接口），
  题库因此越用越厚——这是"AI 帮我把题库变好"的正向循环，也是讲解这条线的复利所在。
