...
- **审阅清单（界面主列表）**：以**题**为单位，每题一行 = 题号 + 题型 + 状态 + 题干 + 当前标签（行内多选下拉，
  **就地改、立刻落库**）+ 来源（你标注的 / AI 建议 带把握分）+ 行内动作（确认 / 丢弃建议 / 看题干 / 详情）。
  「看题干」就地展开题干、选项、答案与解析；「详情」才跳题目编辑器（可选，不是改标签的必经路径）。
- **筛选与批量**：顶部「全部 / 待确认 / 未匹配 / 已确认」是可点击的计数筛选（不是页签），
  另有知识点筛选（下拉里带题量、直接标出"你没有题"= 缺口，并给一行缺口提示）；勾选后可批量设为知识点 / 确认 / 丢弃。
...
- **动作语义**：`confirm` 确认建议 / `set` 把这些题的标签设定为给定节点（就地改标签、批量设为知识点，
  写成 `source=user`，会清掉旧的 AI 建议与旧用户标签，反复设置不累积） / `retag` 按节点批量改挂 /
  `reject` 丢弃建议（已确认的标签不动）。
- **门控与练习**：只有「已确认」或「AI 置信度 ≥ 0.8」的标签参与掌握度判定；低置信标签只作参考。
  **没有标签的题照常进练习与复习调度**，缺标签只影响知识点维度的统计。
- 标签只存本机；导出题库时随包分发（作者确认过的标签），为将来的广场质量分与共享路线做准备。
## 5. 做题、判分与错题
