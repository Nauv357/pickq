...
const { t } = useI18n({
  messages: {
    'zh-CN': {
      ...
      msgSaveAsCreated: '已创建「{name}」，复制 {n} 题（题号已在目标库从 1 重新编排）',
      msgNeedTargetBank: '请选择目标题库',
      msgMergedInto: '已并入「{name}」（复制 {n} 题，题号已顺延编排不重复）',
      msgReviewOnOverdue: '复习计划已开启：待复习 {n} 题（含逾期 {overdue} 题）',
      msgReviewOnToday: '复习计划已开启：今日待复习 {n} 题',
      msgReviewOn: '复习计划已开启',
      msgReviewPaused: '已暂停到期提醒：进度照记、错题照收，只是不再提醒到期复习',
      msgResetReviewConfirm: '将清空该题库的全部复习进度（含单题暂停标记），之后从 1 天间隔重新开始。作答记录与错题本不受影响。确定重置吗？',
      msgResetReviewTitle: '重置复习计划',
      msgResetReviewDone: '已重置复习计划（清除 {n} 条进度）',
      msgNoQuestionsInRange: '该范围没有题目，换个模式试试',
      msgSessionStarted: '已抽取 {n} 题，开始刷题',
      msgSessionStartedAt: '已抽取 {n} 题，从目标题开始刷题',
      msgNoFavorites: '还没有收藏任何题目：点题目行中的星标即可收藏',
      msgFavSessionConfirm: '将以「收藏」模式抽取全部 {n} 道收藏题开始刷题（做题中随时可改收藏，不影响本次会话）。确定开始吗？',
      msgFavSessionTitle: '刷收藏题',
      msgNeedMaterialContent: '请输入材料内容',
      msgMaterialUpdated: '材料已更新',
      msgMaterialCreated: '材料已创建，可在录题时关联',
      msgDeleteMaterialConfirm: '删除后，组内题将解除与该材料的关联。确定删除吗？',
      msgDeleteMaterialTitle: '删除材料',
      msgMaterialDeleted: '材料已删除',
      ...
      msgBatchOk: '批量导入成功，新增 {n} 道题目',
      msgAiFillNone: '本库没有无答案的客观题，无需补答案',
      msgAiFillSummary: '共处理 {n} 道无答案客观题：',
      msgAiFillFilled: '✅ 已回填 {n} 道',
      msgAiFillUndetermined: '❓ 无法判定 {n} 道（选项不完整或模型无法确定，可手动补）',
      msgAiFillFailed: '⚠️ 批次失败 {n} 道（可稍后重试）',
      msgAiFillDoneTitle: 'AI 补答案完成',
      msgLastQuestion: '已经是最后一题了',
      msgFirstQuestion: '已经是第一题了',
      msgDeleteBankConfirm: '确定删除题库「{name}」吗？将同时删除其下 {n} 道题目和全部刷题记录，此操作不可恢复。',
      msgDeleteBankTitle: '删除题库',
      msgBankDeleted: '题库已删除（连带删除 {n} 道题目、{m} 条刷题记录）',
    ...
    'en-US': {
      ...
      msgSaveAsCreated: 'Created “{name}” and copied {n} questions (numbers were renumbered from 1 in the target bank)',
      msgNeedTargetBank: 'Please select a target bank',
      msgMergedInto: 'Merged into “{name}” ({n} questions copied; numbers continue without duplicates)',
      msgReviewOnOverdue: 'Review plan enabled: {n} due (including {overdue} overdue)',
      msgReviewOnToday: 'Review plan enabled: {n} due today',
      msgReviewOn: 'Review plan enabled',
      msgReviewPaused: 'Due reminders paused: progress and mistakes are still recorded — you just won\'t be reminded about due reviews',
      msgResetReviewConfirm: 'This will clear all review progress for this bank (including per-question pause flags) and restart from a 1-day interval. Answer records and the mistake book are unaffected. Reset anyway?',
      msgResetReviewTitle: 'Reset review plan',
      msgResetReviewDone: 'Review plan reset ({n} progress records cleared)',
      msgNoQuestionsInRange: 'No questions in this range — try a different mode',
      msgSessionStarted: '{n} questions drawn, let\'s start',
      msgSessionStartedAt: '{n} questions drawn, starting from the target question',
      msgNoFavorites: 'No favorites yet: click the star on a question row to favorite it',
      msgFavSessionConfirm: 'This will start a favorites session with all {n} favorited questions (you can change favorites anytime — it won\'t affect this session). Start?',
      msgFavSessionTitle: 'Practice favorites',
      msgNeedMaterialContent: 'Please enter material content',
      msgMaterialUpdated: 'Material updated',
      msgMaterialCreated: 'Material created — you can link it when adding questions',
      msgDeleteMaterialConfirm: 'After deleting, questions in the group will no longer be linked to this material. Delete it?',
      msgDeleteMaterialTitle: 'Delete material',
      msgMaterialDeleted: 'Material deleted',
      ...
      msgBatchOk: 'Batch import succeeded, {n} questions added',
      msgAiFillNone: 'This bank has no unanswered objective questions — nothing to fill',
      msgAiFillSummary: 'Processed {n} unanswered objective questions:',
      msgAiFillFilled: '✅ {n} filled in',
      msgAiFillUndetermined: '❓ {n} undetermined (incomplete options or the model couldn\'t decide — fill manually)',
      msgAiFillFailed: '⚠️ {n} failed in this batch (retry later)',
      msgAiFillDoneTitle: 'AI answer filling complete',
      msgLastQuestion: 'This is already the last question',
      msgFirstQuestion: 'This is already the first question',
      msgDeleteBankConfirm: 'Delete the bank “{name}”? This will also delete its {n} questions and all practice records. This cannot be undone.',
      msgDeleteBankTitle: 'Delete bank',
      msgBankDeleted: 'Bank deleted (also removed {n} questions and {m} practice records)',
...
async function submitSaveAs() {
  if (selectedQids.size > MAX_SELECT) {
    ElMessage.warning(t('msgMaxCopy', { n: MAX_SELECT }))
    return
  }
  ...
  try {
    ...
    ElMessage.success(t('msgSaveAsCreated', { name: res.name, n: res.questionsCopied }))
...
async function submitMergeTo() {
  ...
  if (!mergeTargetBankId.value) {
    ElMessage.warning(t('msgNeedTargetBank'))
    return
  }
  selectionBusy.value = true
  try {
    ...
    ElMessage.success(t('msgMergedInto', { name: res.name, n: res.questionsCopied }))
  ...
  try {
    ...
    if (next) {
      ...
      if (dueTotal.value > 0) {
        ElMessage.success(
          overdue > 0
            ? t('msgReviewOnOverdue', { n: dueTotal.value, overdue })
            : t('msgReviewOnToday', { n: dueTotal.value })
        )
      } else {
        ElMessage.success(t('msgReviewOn'))
      }
    } else {
      dueTotal.value = 0
      ElMessage.info(t('msgReviewPaused'))
    }
  } catch (e) {
    /* 拦截器已提示 */
  } finally {
...
async function resetReviewPlan() {
  ...
  try {
    ...
    ElMessage.success(t('msgResetReviewDone', { n }))
    dueTotal.value = 0
    dueList.value = []
    dueVisible.value = false
  } catch (e) {
...
async function startSessionWith(body, expectedTotal) {
  ...
  try {
    ...
    if (!data.total || data.questions.length === 0) {
      ElMessage.warning(t('msgNoQuestionsInRange'))
      return
    }
...
async function startFavoriteSession() {
  ...
  if (favoriteCount.value === 0) {
    ElMessage.info(t('msgNoFavorites'))
    return
  }
  try {
    await ElMessageBox.confirm(
...
async function saveMaterial() {
  ...
  if (!content) {
    ElMessage.warning(t('msgNeedMaterialContent'))
    return
  }
  ...
  try {
    if (editingMaterialId.value) {
      ...
      ElMessage.success(t('msgMaterialUpdated'))
    } else {
      await createMaterial(id, content)
      ElMessage.success(t('msgMaterialCreated'))
    }
    resetMaterialForm()
    loadMaterials()
  } catch (e) {
...
async function removeMaterial(m) {
  ...
  try {
    ...
    ElMessage.success(t('msgMaterialDeleted'))
    loadMaterials()
  } catch (e) {
    /* 拦截器已提示 */
  }
...
async function startPracticeAt(questionId) {
  ...
  try {
    ...
    if (!data.total || data.questions.length === 0) {
      ElMessage.warning(t('noQuestions'))
      return
    }
    ElMessage.success(t('msgSessionStartedAt', { n: data.total }))
    router.push({ path: `/banks/${id}/practice`, query: { sessionId: data.sessionId } })
  } catch (e) {
    /* 拦截器已提示 */
  } finally {
...
async function handleNavigate(dir) {
  ...
  try {
    ...
    ElMessage.info(dir > 0 ? t('msgLastQuestion') : t('msgFirstQuestion'))
  } finally {
...
async function submitEditBank() {
  ...
  try {
    ...
    if (Object.keys(patch).length === 0) {
      ElMessage.info(t('msgNoChanges'))
      editBankVisible.value = false
      return
    }
    await updateBank(id, patch)
    ElMessage.success(t('msgBankUpdated'))
    editBankVisible.value = false
    await loadBank()
  } catch (e) {
    /* 拦截器已提示 */
...
async function submitExport() {
  ...
  try {
    ...
    if (exportForm.format === 'tiku') {
      ...
      if (res && !res.saved) return //桌面版用户取消另存为
      ElMessage.success(res?.path ? t('msgExportedTo', { path: res.path }) : t('msgExportedJson', { file: baseName }))
    }
...
async function submitAiFill() {
  ...
  try {
    ...
    if (total === 0) {
      ElMessage.info(t('msgAiFillNone'))
    } else {
      const parts = [t('msgAiFillSummary', { n: total })]
      parts.push(t('msgAiFillFilled', { n: filled }))
      if (undetermined > 0) parts.push(t('msgAiFillUndetermined', { n: undetermined }))
...
async function confirmDeleteBank() {
  ...
  try {
    ...
    ElMessage.success(t('msgBankDeleted', { n: deletedQuestions, m: affectedRecords }))
    router.push('/')
  } catch (e) {
    /* 拦截器已提示 */
  }
...
async function confirmDeleteQuestion(q) {
  ...
  try {
    ...
    ElMessage.success(t('msgQuestionDeleted'))
    // 当前页删空则回退一页
