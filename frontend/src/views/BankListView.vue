  ...
  <div class="page">
    ...
    <template v-else>
      ...
      <div v-else class="grid">
        ...
        >
          <!-- 卡片只有三层：标题 / 题数与进度 / 元信息（版本·作者·时间）。
               操作入口不常显（常显会让卡片显得杂乱）：悬停或键盘聚焦时出现，右键随时可用。 -->
          <div class="bank-head">
          ...
          <p v-if="bankNote(bank)" class="bank-desc" :title="bankNote(bank)">{{ bankNote(bank) }}</p>

          <div class="bank-stat">
            <span><b class="stat-num mono">{{ bank.questionCount ?? 0 }}</b> {{ t('meta.qUnit') }}</span>
            <template v-if="answeredOf(bank) > 0">
              <span class="meta-dot"></span>
              <span>{{ t('meta.done') }} <b class="stat-num mono">{{ answeredOf(bank) }}</b></span>
            </template>
          </div>

          <div class="bank-sub text-muted" :title="fullMetaText(bank)">
            <template v-for="(part, pi) in subParts(bank)" :key="part">
              <span v-if="pi > 0" class="meta-dot"></span>
              <span class="bank-sub-part">{{ part }}</span>
            </template>
          </div>
        </div>
      </div>
...
import { createSession, getPracticePlan } from '../api/sessions'
...
const { t } = useI18n({
  messages: {
    'zh-CN': {
      ...
      act: { import: '导入', merge: '合并题库', create: '创建题库' },
      choice: {
        title: '导入',
        lead: '按你的情况选一种：',
        aiTitle: '我手上有试卷 / 学习资料',
      ...
      guideNoRecord: '还没有做题记录：任一题库卡片上点右键（或「…」）→「开始练习」，刷完第一场后这里会显示你的练习概况与复习提醒',
      msgNoQuestions: '这个题库还没有可练的题',
      filterEmpty: '没有匹配的题库，换个关键词试试',
      createdOn: '创建于 {d}',
      noDesc: '暂无描述', genByAi: '由 AI 导入生成',
      ago: { justNow: '刚刚', minutes: '{n} 分钟前', hours: '{n} 小时前', days: '{n} 天前' },
      dialogs: {
        createTitle: '创建题库',
        name: '题库名称',
        namePh: '例如：C1 驾考科目一',
    ...
    'en-US': {
      ...
      act: { import: 'Import', merge: 'Merge Banks', create: 'Create Bank' },
      choice: {
        title: 'Import',
      ...
      guideNoRecord: 'No practice records yet: right-click a bank card (or use its “…” menu) → “Start practice” — your overview will appear here after your first round.',
      msgNoQuestions: 'This bank has no questions to practice yet',
      filterEmpty: 'No banks match your search. Try other keywords.',
      createdOn: 'Created {d}',
      noDesc: 'No description', genByAi: 'Generated by AI import',
      ago: { justNow: 'just now', minutes: '{n} min ago', hours: '{n} h ago', days: '{n} d ago' },
      dialogs: {
    ...
    msgImportingFile: 'Importing bank…',
      ...
      meta: { qUnit: 'questions', done: 'attempted' },
      menu: {
...
/* ---------- 卡片信息分层（标题 / 题数 / 元信息），保持每张卡片行数一致且不折行 ---------- */

/**
 * 卡片副标题：只显示**用户自己写的**描述。
 * 系统占位描述（"由 AI 导入生成"）在几乎每张卡片上都出现，属于纯噪音，卡片里不再显示
 * （详情页仍会展示），这样有描述与没描述的卡片行高也一致。
 */
function bankNote(b) {
  const d = b?.description
  if (!d || d === AI_GEN_DESC) return ''
  return d
}

function answeredOf(b) {
  return Number(b?.answeredCount || 0)
}

/**
 * 元信息行（小字、单行省略）：版本 · 作者 · 创建时间。
 * 时间用相对时间（"3 天前"）而不是完整时间戳，`title` 里给完整时间。
 */
function subParts(b) {
  const parts = []
  if (b?.version) parts.push(`v${b.version}`)
  if (b?.authorName) parts.push(b.authorName)
  const ago = fmtAgo(b?.createdAt)
  if (ago) parts.push(ago)
  return parts
}

function fullMetaText(b) {
  const full = t('createdOn', { d: formatDate(b?.createdAt) })
  return b?.authorName ? `${full} · ${b.authorName}` : full
}
...
function fmtAgo(iso) {
  ...
  if (diff < 60 * 1000) return t('ago.justNow')
  if (diff < 3600 * 1000) return t('ago.minutes', { n: Math.floor(diff / 60000) })
  if (diff < 24 * 3600 * 1000) return t('ago.hours', { n: Math.floor(diff / 3600000) })
  if (diff < 7 * 24 * 3600 * 1000) return t('ago.days', { n: Math.floor(diff / (24 * 3600000)) })
...
async function onBankMenuSelect(key) {
  ...
  if (key === 'practice') return startPractice(bank)
...
/**
 * 卡片菜单「开始练习」：与题库详情页的主按钮**同一条路径**（一键配题 → 直接进做题页）。
 * 词与行为都统一（docs/design-ui.md R6/R14）：范围与题量想自己定，进题库页 →「更多 → 自定义练习…」。
 */
const practiceStarting = ref(null)

async function startPractice(bank) {
  if (practiceStarting.value) return
  practiceStarting.value = bank.id
  try {
    const plan = await getPracticePlan(bank.id, { count: 20 })
    if (!plan?.items?.length) {
      ElMessage.warning(t('msgNoQuestions'))
      return
    }
    ...
    try {
      sessionStorage.setItem(`tiku.plan.${session.sessionId}`, plan.explain || '')
    } catch (e) {
      /* 写不了也不影响开练 */
    }
    router.push({ path: `/banks/${bank.id}/practice`, query: { sessionId: session.sessionId } })
  } catch (e) {
    /* 拦截器已提示 */
  } finally {
    practiceStarting.value = null
  }
}
...
/* 标题行：标题占满左侧，右侧固定一个 24px 槽位放「…」/勾选框（槽位常驻，标题不会钻到按钮下面） */
.bank-head {
  display: flex;
  align-items: center;
  gap: 8px;
...
/* 卡片层级：标题行 →（可选）用户描述 → 题数/进度 → 元信息（版本·作者·时间）
   行距用 gap 控制，元素自身不带 margin，避免"忽宽忽窄"的间距错觉 */
.bank-desc {
  margin: 0;
  color: var(--text-secondary);
  font-size: 13px;
