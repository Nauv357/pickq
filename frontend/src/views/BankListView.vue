...
async function submitMerge() {
  ...
  try {
    ...
    ElMessage.success(`合并完成：${res.questionsCopied} 道题已复制进「${res.name}」（源题库保留，题号已从 1 重新编排）`)
...
const IMPORT_TIPS = {
  CREATED: '导入成功',
  ALREADY_IMPORTED: '该题库文件已导入过',
  VERSION_ADDED: '已并存导入新版本',
  BRANCHED: '内容已被修改，已作为新题库文件导入（分支）'
}
...
async function handleImportResult(res) {
  const tip = IMPORT_TIPS[res?.result] || '导入完成'
  if (res?.result === 'CREATED') {
    ElMessage.success(`${tip}：${res.message || ''}`)
    await loadBanks()
    try {
      await ElMessageBox.confirm(`导入成功，是否跳转到新题库「${res.bankId}」？`, '导入完成', {
    ...
    ElMessage.warning(`${tip}：${res.message || ''}`)
    await loadBanks()
    try {
      await ElMessageBox.confirm('是否跳转到原题库？', '已导入过', {
        confirmButtonText: '去查看',
        cancelButtonText: '取消'
      })
      router.push(`/banks/${res.bankId}`)
